...
async function createTempSourceFile(filename: string) {
  const dir = await fs.mkdtemp(path.join(tmpdir(), 'devgrove-'));
            ...
            for (const child of children) {
              ...
              if (importLine) {
                ...
                componentImports.set(
                  importLine.module,
                  importLine?.isDefault ? newImports : [...newImports],
                );
              }
            }

            const componnetImportLines = [...componentImports.keys()].map(
              (key) => {
  ...
  --ring: <%- colors.dark["ring"] %>;
    ...
    for (const baseColor of ['slate', 'gray', 'zinc', 'neutral', 'stone']) {
      ...
      for (const [mode, values] of Object.entries(colorMapping)) {
        ...
        for (const [key, value] of Object.entries(values)) {
          if (typeof value === 'string') {
            const resolvedColor = value.replaceAll(
              '{{base}}-',
              `${baseColor}-`,
            );
